// ====================================================================
// 🛡️ GAME-CORE.JS — Reino do Caos (com eventos + atributos + level up + equiped + vestimentas + NPCs)
// ====================================================================
const GAME_CONFIG = {
...
function requireAuth() {
  ...
  if (!idusuario || !isValidUUID(idusuario)) {
...
async function fetchPlayerFromDB(idplayer) {
  if (!idplayer) throw new Error('ID do jogador é obrigatório.');
  
  const { data: player, error: playerErr } = await supabase
    .from('player')
    .select('nome, classe, level, vida, maxvida, energia, ouro, exp, forcaini, defesaini, inteligenciaini, idclasse, infamia, idtitulofk')
  ...
  if (player.idtitulofk) {
    const { data: tituloData, error: tituloErr } = await supabase
      .from('titulos')
      .select('descricao')
      .eq('idtitulo', player.idtitulofk)
      .single();
    if (!tituloErr && tituloData?.descricao) {
      titulo = tituloData.descricao;
    }
  }

  return { ...player, titulo };
}

function savePlayerToStorage(player) {
  const safeData = {
    nomeplayer: player.nome,
    classe: player.classe, // ✅ LINHA ADICIONADA
    level: player.level || 1,
    vida: player.vida || 0,
    maxvida: player.maxvida || 0,
    energia: player.energia || 0,
    ouro: player.ouro || 0,
    exp: player.exp || 0,
    forcaini: player.forcaini || 0,
    defesaini: player.defesaini || 0,
    inteligenciaini: player.inteligenciaini || 0,
    idclasse: player.idclasse,
    infamia: player.infamia || 0,
    titulo: player.titulo || '-' // ✅ salva o título
  };
  localStorage.setItem('playerData', JSON.stringify(safeData));
  return safeData;
}
async function ensurePlayerData() {
  let player = JSON.parse(localStorage.getItem('playerData')) || null;
  if (!player) {
    const idplayer = localStorage.getItem('idplayer');
    if (idplayer) {
      const dbPlayer = await fetchPlayerFromDB(idplayer);
      player = savePlayerToStorage(dbPlayer);
    }
  }
  return player;
}
...
// ====================================================================
// 🔐 Criação de Conta e Personagem (para createplayer.html)
// ====================================================================
...
// ====================================================================
// 📚 Missões, Eventos, Classes (mantidos como no original)
// ====================================================================
...
// Armas — ✅ CORRIGIDO: subtrai o bônus real da arma equipada
async function equipWeapon(armaId) {
  const idplayer = localStorage.getItem('idplayer');
  if (!idplayer || !isValidUUID(idplayer)) throw new Error('ID do jogador inválido.');
  if (!armaId || !isValidUUID(armaId)) throw new Error('ID da arma inválido.');
...
async function unequipWeapon() {
  ...
  if (!armaEquipada) {
    // Nenhuma arma equipada? Só desmarca todas (defensivo)
    await supabase
      .from('player-arma')
  ...
  // 2. Ler o forcaini ATUAL do jogador
  const player = JSON.parse(localStorage.getItem('playerData')) || {};
  const forcainiAtual = player.forcaini || 0;
...
// Vestimentas — ✅ CORRIGIDO: subtrai os bônus reais da vestimenta equipada
async function equipVestimenta(vestId) {
  const idplayer = localStorage.getItem('idplayer');
  if (!idplayer || !isValidUUID(idplayer)) throw new Error('ID do jogador inválido.');
  if (!vestId || !isValidUUID(vestId)) throw new Error('ID da vestimenta inválido.');
...
async function unequipVestimenta() {
  ...
  // 1. Encontrar a vestimenta equipada
...
// ====================================================================
// 🛌 Dormir — restaura vida e energia ao máximo
// ====================================================================
async function dormir() {
  ...
  const player = JSON.parse(localStorage.getItem('playerData')) || {};
  const updates = {    
    vida: player.maxvida || 100,     // ✅ restaura vida ao máximo
    energia: 100   // ✅ restaura energia (ou use 100 fixo)
  };
  await updatePlayer(updates);
  return updates;
}
...
window.GAME = {
  config: GAME_CONFIG,
  // expose supabase so other modules/pages can use it directly
  supabase,
  requireAuth,
  logout,
  isValidUUID,
  ensurePlayerData,
  savePlayerToStorage, // <--- exportada para permitir selectPlayer() usar
  fetchPlayerFromDB,
  updatePlayer,
  loadMissions,
  executeMission,
