...
function savePlayerToStorage(player) {  
  const safeData = {
     // ✅ Salva no localStorage com a DESCRIÇÃO do título (não o ID)  
    nomeplayer: data.nome,
    classe: data.classe,
    level: data.level || 1,
    vida: data.vida || 0,
    maxvida: data.maxvida || 0,
    energia: data.energia || 0,
    ouro: data.ouro || 0,
    exp: data.exp || 0,
    forcaini: data.forcaini || 0,
    defesaini: data.defesaini || 0,
    inteligenciaini: data.inteligenciaini || 0,
    idclasse: data.idclasse,
    infamia: data.infamia || 0,
    titulo: descricaoTitulo // ✅ Aqui vai a descrição, não o ID!
  };
...
async function atualizarTituloPlayer() {
  const idplayer = localStorage.getItem('idplayer');
  if (!idplayer) return;
  const dbPlayer = fetchPlayerFromDB(idplayer);
  const current = JSON.parse(localStorage.getItem('playerData')) || {};
  const updated = { ...current, titulo: dbPlayer.titulo };
...
// ====================================================================
async function createPlayerGm(nome, idclasse) {
  const idusuario = localStorage.getItem('idusuario');
  ...
  // ✅ Busca o primeiro título
  let idtitulofk = null;
  try {
    const { data: primeiroTitulo } = await supabase
      .from('titulos')
      .select('idtitulo')
      .order('idtitulo', { ascending: true })
      .limit(1)
      .single();
    if (primeiroTitulo) idtitulofk = primeiroTitulo.idtitulo;
  } catch (e) {
    console.warn('Nenhum título encontrado:', e);
  }
  ...
  const novo = {
    ...
    idtitulofk: idtitulofk // ✅ CORRIGIDO!
  };
...
// Define o jogador como ONLINE
async function setPlayerOnline() {
  const idusuario = localStorage.getItem('idusuario');
  if (!idusuario) return;
  try {
    await supabase
    ...
    console.warn('⚠️ Não foi possível atualizar status online:', err.message);
  }
}

// Define o jogador como OFFLINE
async function setPlayerOffline() {
  ...
  try {
    ...
    console.warn('⚠️ Não foi possível atualizar status offline:', err.message);
  }
}
...
async function dormir() {
  ...
  const player = JSON.parse(localStorage.getItem('playerData')) || {};
  const updates = {    
    vida: player.vida*0.3+player.vida || 100,
    energia: 100
  };
  await updatePlayer(updates);
  return updates;
